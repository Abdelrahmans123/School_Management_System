    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">{{trans('grade.deleteGrade')}}</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <div class="modal-body">
                <form action="{{route('grade.destroy','test')}}" method="POST" class="needs-validation   @error('gradeAR') was-validated @enderror" novalidate >
                    @csrf
                    @method('DELETE')
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-danger">{{trans('grade.deleteGrade')}}</button>
                    </div>
                </form>
            </div>
