...
@section('css')
@endsection
@section('title', trans('library.edit'))


@section('headerTitle', trans('library.edit'))
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <div class="container">
                <form action="{{ route('library.update', 'test') }}" method="POST" autocomplete="off" class="needs-validation"
                    ...
                    <div class="row">
                            ...
                            <select name="teacher_id" class="form-select" id="parentInput" required>
                                ...
                                @foreach ($teachers as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $item->id == $book->teacher_id ? 'selected' : '' }}>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                            @error('teacher_id')
                        ...
                        <div class="col-lg-4 mt-2 has-validation">
                            ...
                            <select name="stage_id" class="form-select" id="stageInput" required>
                                ...
                                @foreach ($stages as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $item->id == $book->stage_id ? 'selected' : '' }}>{{ $item->Name }}</option>
                                @endforeach
                            </select>
                            @error('stage_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-lg-4 mt-2  has-validation">
                            ...
                            <select name="section_id" class="form-select" id="sectionInput" required>
                                <option selected value="{{ $book->section_id }}">{{ $book->sections->name }}</option>
                            </select>
                            @error('section_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-lg-12 mt-2  has-validation">
							<div class="col-lg-12 mt-2  has-validation">
								<embed src="{{ URL::asset('attachments/books/' . $book->file_name) }}"
									type="application/pdf" height="200px" width="200px">
							</div>
